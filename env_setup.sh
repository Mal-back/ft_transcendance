...
VAULT=./srcs/requirements/vault/certs
...
openssl req -new -newkey rsa:4096 -nodes -out $VAULT/vault.csr -keyout $VAULT/vault.key -subj "/C=FR/ST=IDF/L=PARIS/O=42/OU=42/CN=vault/UID=thole"
openssl x509 -req -days 365000 -in $VAULT/vault.csr -out $VAULT/vault.crt -CA ca.crt -CAkey ca.key
